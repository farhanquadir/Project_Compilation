...
from keras.layers import Dense, Activation, Flatten, Dropout, BatchNormalization, Embedding
...
training_list_file="Proble4Dataset_large_train__.csv"
#test_list_file="HW5_test.csv"
test_list_file="Proble4Dataset_large_test__.csv"
...
print (max(l))
max_len=max(l)
token_train_x=Tokenizer(num_words=200)#.fit_on_texts(train_x)
...
word_model_train = gensim.models.Word2Vec(sentences, size=200, min_count = 1, window = 5)

documents = test_x
sentences = [[word for word in document.lower().split()] for document in documents]

word_model_test = gensim.models.Word2Vec(sentences, size=200, min_count = 1, window = 5)
"""
...
#Tokenize test text
token_test_x=Tokenizer(num_words=200)#.fit_on_texts(train_x)
...
print(predictions)
print(model.evaluate(train_x,train_y,batch_size=1000))
tp_00=0
tp_11=0
fp_01=0
fp_10=0
