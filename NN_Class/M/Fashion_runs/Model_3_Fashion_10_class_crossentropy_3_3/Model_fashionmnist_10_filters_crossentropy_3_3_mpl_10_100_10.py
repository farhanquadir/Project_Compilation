...
class CNN(nn.Module):
    ...
    def __init__(self, size, num_classes):
        ...
        #self.decimate = nn.Sequential( nn.Linear(10,100), nn.ReLU(inplace=True),nn.Linear(100,10),nn.Sigmoid())
        self.decimate = nn.Sequential(nn.Sigmoid())
        """
...
train = datasets.FashionMNIST( root = '../',
...
for epoch in range(num_epochs): # how many epochs? 
    ...
    for batch_id, train_params in enumerate(tqdm(train_ld)):  # lets grab a bunch of mini-batches from our training data set
        ...
        ll=torch.zeros(1,10,device=device).type("torch.cuda.LongTensor")
...
for sample, label in tqdm(test):
    ...
    # take the largest output and return integer of which it was (make a classification decision)
    #if (label==0):
    #    prediction=int(torch.argmin(prediction).numpy())
    #else:
    #    prediction = int(torch.argmax(prediction).numpy())
    prediction = int(prediction.argmax())
    # what was our prediction?
    #print(prediction)
    if label==prediction: total_acc+=1
    ConfusionMatrix[label,prediction] = ConfusionMatrix[label,prediction] + 1
...
#plt.show()
plt.close()
print("Total accuracy = ",100*total_acc/len(test.dataset.data))
