...
for epoch in range(num_epochs): # how many epochs? 
    ...
    for batch_id, train_params in enumerate(tqdm(train_ld)):  # lets grab a bunch of mini-batches from our training data set
        ...
        #samples = samples.type('torch.FloatTensor',device=device) 
        #labels = labels.type('torch.LongTensor')  
        labs=int(labels.item())
        #print("Labs=",labels.item(),type(labels.item()))
        if (int(labels.item())==c1):
            ll.data[0][0]=1#label_dict[labels.item()]
        else:
...
#        sys.exit()
        ...
        #print (np.max(prediction.data.cpu().numpy()))
        prediction_acc = int(prediction.argmax())#int(np.max(prediction.data.cpu().numpy())+0.5)
        #print("index=",prediction_acc)
        #print ("ASDFADSF")
        #print (labels,prediction,label_acc,prediction_acc)
        #sys.exit()
    ...
    for sample, labels in tqdm(test):
        ...
        #labels = labels.type('torch.LongTensor')  
        label_acc = label_dict[int(labels.numpy())]
...
for sample, label in tqdm(test):
    ...
    #labs=label.copy()
    labels=label
    label = label_dict[int(label.item())]
    labs = label
    #print("Real label is")
    #print(label)
    # convert the sample (image) to a tensor for PyTorch
    ...
    prediction = int(prediction.argmax())#int(np.max(prediction.data.cpu().numpy())+0.5)
...
df_cm = pd.DataFrame(np.asarray(ConfusionMatrix,dtype=int), index = [i for i in ["1","3"]],
                  columns = [i for i in ["1","3"]])
