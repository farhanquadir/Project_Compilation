...
for epoch in range(num_epochs): # how many epochs? 
    ...
    for batch_id, train_params in enumerate(tqdm(train_ld)):  # lets grab a bunch of mini-batches from our training data set
        ...
        ll.data[0][0]=label_dict[labels.item()]
        labels=ll
        #print (samples[0])
        #print (labels[0])
        ...
        label_acc = label_dict[int(labs)]#int(labels.numpy())
        prediction_acc = prediction.item()
        if (prediction_acc>0.5):
            prediction_acc=int(1)
        else:
            prediction_acc=0.5#int(0)
        if (label_acc==prediction_acc): train_acc_count+=1
        if prediction.item()<0.5: os.system("echo '"+str(prediction.item())+"' >> pred.txt")
...
for sample, label in test:
    ...
    # take the largest output and return integer of which it was (make a classification decision)
    prediction = predictions.item()#int(torch.argmax(prediction).numpy())
    ...
    if (prediction>0.5):
        ...
        prediction=int(0)
    pred=prediction
    if (label_dict[label]==prediction): 
        val_acc+=1
