...
class CNN(nn.Module):
    ...
    def __init__(self, size, num_classes):
        ...
        self.extract = nn.Sequential( # lets make a 2D convolution layer
                                      ...
                                      nn.ReLU(inplace = True),#,nn.Sigmoid() 28 x 28 x 10
                                      #nn.MaxPool2d(2),                                                                
                                      #nn.Dropout(0.1),
                                      # ----------------------------------------------------------- 
                                      # now, lets make another layer of convolution, pooling, and drop out
                                      nn.Conv2d( in_channels = 10, out_channels = 4, #26 x 26 x 10 x 4
        ...
        self.decimate = nn.Sequential( nn.Linear(26*26*4,512), nn.ReLU(inplace=True), nn.Linear(512,128), nn.ReLU(inplace=True),
                                      nn.Linear(128,64), nn.ReLU(inplace=True),
                                      nn.Linear(64,10), nn.ReLU(inplace=True), nn.Linear(10,3), nn.Softmax() 
                                      )#,nn.Sigmoid())
        """
