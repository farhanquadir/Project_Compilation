    ...
    def __init__(self, size, num_classes):
        ...
        self.extract = nn.Sequential( # lets make a 2D convolution layer
                                      nn.Conv2d( in_channels = size, out_channels = 10, 
                                                 kernel_size = (11,11), stride = 1, padding = 0), 
                                      nn.ReLU(inplace = True),#,nn.Sigmoid() 18 x 18 x 10
                                      #nn.MaxPool2d(2),                                                                
                                      #nn.Dropout(0.1),
                                      # ----------------------------------------------------------- 
                                      # now, lets make another layer of convolution, pooling, and drop out
                                      nn.Conv2d( in_channels = 10, out_channels = 4, #8 x 8 x 10 x 4
        ...
        self.decimate = nn.Sequential( nn.Linear(8*8*4,512), nn.ReLU(inplace=True),# nn.Linear(512,128), nn.ReLU(inplace=True),
