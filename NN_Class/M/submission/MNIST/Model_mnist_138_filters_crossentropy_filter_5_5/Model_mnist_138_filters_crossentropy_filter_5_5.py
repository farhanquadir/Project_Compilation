...
class CNN(nn.Module):
    ...
    def __init__(self, size, num_classes):
        ...
        self.extract = nn.Sequential( # lets make a 2D convolution layer
                                      ...
                                      nn.ReLU(inplace = True),#,nn.Sigmoid() 24 x 24 x 10
                                      #nn.MaxPool2d(2),                                                                
                                      #nn.Dropout(0.1),
                                      # ----------------------------------------------------------- 
                                      # now, lets make another layer of convolution, pooling, and drop out
                                      nn.Conv2d( in_channels = 10, out_channels = 4, #20 x 20 x 10 x 4
        ...
        self.decimate = nn.Sequential( nn.Linear(20*20*4,512), nn.ReLU(inplace=True),# nn.Linear(512,128), nn.ReLU(inplace=True),
...
# how did we do...
val_acc=0
ConfusionMatrix = torch.zeros((3,3),dtype=int)
i=0
for sample, label in tqdm(test):
    i+=1
    # what is its label?
