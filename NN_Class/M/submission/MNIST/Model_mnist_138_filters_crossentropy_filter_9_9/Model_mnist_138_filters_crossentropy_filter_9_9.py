    ...
    def __init__(self, size, num_classes):
        ...
        self.extract = nn.Sequential( # lets make a 2D convolution layer
                                      nn.Conv2d( in_channels = size, out_channels = 10, 
                                                 kernel_size = (9,9), stride = 1, padding = 0), 
                                      nn.ReLU(inplace = True),#,nn.Sigmoid() 20 x 20 x 10
                                      #nn.MaxPool2d(2),                                                                
                                      #nn.Dropout(0.1),
                                      # ----------------------------------------------------------- 
                                      # now, lets make another layer of convolution, pooling, and drop out
                                      nn.Conv2d( in_channels = 10, out_channels = 4, #12 x 12 x 10 x 4
        ...
        self.decimate = nn.Sequential( nn.Linear(12*12*4,512), nn.ReLU(inplace=True),# nn.Linear(512,128), nn.ReLU(inplace=True),
                                      #nn.Linear(128,64), nn.ReLU(inplace=True),
                                      nn.Linear(512,10), nn.ReLU(inplace=True), nn.Linear(10,3), nn.Softmax() 
                                      )#,nn.Sigmoid())
        """
...
for epoch in range(num_epochs): # how many epochs? 
    ...
    for batch_id, train_params in enumerate(tqdm(train_ld)):  # lets grab a bunch of mini-batches from our training data set
        ...
        #ll=torch.zeros(1,3)
        ll=torch.zeros(1).type('torch.cuda.LongTensor')
        ...
        ll.data[0]=int(label_dict[labels.item()])
        labels=ll
        #print (samples[0])
        #print (labels[0])
...
#        sys.exit()
        ...
        # do our update
        optimizer.step()
        label_acc = int(label_dict[labs])#int(torch.argmax(labels).numpy())
        #prediction_acc = int(torch.cuda.argmax(prediction).numpy())
        pred=prediction.argmax()#torch.Tensor(prediction)
        prediction_acc = int(pred)#int(torch.argmax(pred).numpy())
    ...
    for sample, labels in tqdm(test):
        ...
        label_acc = label_dict[labs]#int(torch.argmax(labels).numpy())
        if (label_acc==prediction_acc): 
            val_acc_count+=1
...
for sample, label in tqdm(test):
    ...
    #print ("Here")
    #print (label)
    label = int(label_dict[int(label.item())])#int(label.numpy())
    #print (label)
    #print("Real label is")
    #print(label)
    # convert the sample (image) to a tensor for PyTorch
...
df_cm = pd.DataFrame(np.asarray(ConfusionMatrix), index = [i for i in "138"],
                  columns = [i for i in "138"])
