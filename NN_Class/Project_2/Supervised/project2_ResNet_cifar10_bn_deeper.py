...
def residualBlock(input_var):
    def f(input):
        ...
        l1 = Conv2D(64, kernel_size=3, padding="same") (input)
#        bn1= BatchNormalization(axis=-1)(l1)
        bn1= BatchNormalization()(l1)
        relu1 = Activation("relu")(bn1)
        #l2 =Conv2D(32, kernel_size=3, padding="same",activation="relu") (l1)
        l2 =Conv2D(32, kernel_size=3, padding="same") (relu1)
        relu2 = Activation("relu")(l2)
#        bn2= BatchNormalization(axis=-1)(relu2)
        bn2= BatchNormalization()(relu2)
        
        residual_shape = K.int_shape(bn2)
...
def saveModel(model):
    model.save("ResNet_cifar10_10block_bn.h5")
    model.save_weights("ResNet_cifar10_weights_10block_bn.h5")
    return
...
path=os.getcwd()
modelsave=path+"/model_ResNet_cifar10_10_block_bn_.h5"
model = getModel()
model.summary()
...
with open ("history_ResNet_cifar10_10block_bn.txt","w") as f:
