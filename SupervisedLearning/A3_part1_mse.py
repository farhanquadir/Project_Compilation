...
s_data=loadData(workdir+s_data)
v_data=loadData(workdir+v_data)
ones=np.ones((len(s_data),1))
s_data=np.append(ones,s_data,axis=1)
v_data=np.append(ones,v_data,axis=1)

#print(s_data)

s_train,s_test = getTestTrain(s_data)

v_train,v_test = getTestTrain(v_data)



#create X dataset for all datas
X_train = np.concatenate((s_train,v_train))
X_test = np.concatenate((s_test,v_test))
...
#Y = np.array([])
Y = []
for i in range(len(s_train)+len(v_train)):
    if i < len(s_train):
        Y.append(l1)
    else:
        Y.append(l3)
...
for i in range(len(s_test)+len(s_test)):
    if i < len(s_test):
        Y.append(l1)
    else:
        Y.append(l3)
...
for out_epochs in range (1):
    ...
    w1 = np.random.uniform(-1,1,(5)) # range [-1,1]
